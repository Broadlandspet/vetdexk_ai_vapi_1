...
//         // Get calls count for this hospital
//         const totalCallsResult = await executeQuery(`SELECT COUNT(*) FROM calls WHERE hospital_id = $1`, [hospitalId]);
//         const totalCalls = parseInt(totalCallsResult.rows[0].count);

//         // Get completed calls count for this hospital
//         const completedCallsResult = await executeQuery(
//             `SELECT COUNT(*) FROM calls WHERE call_status = 'completed' AND hospital_id = $1`,
//             [hospitalId]
//         );
//         const completedCalls = parseInt(completedCallsResult.rows[0].count);

//         // Get conversations count for this hospital
//         const conversationsResult = await executeQuery(`SELECT COUNT(*) FROM conversations WHERE hospital_id = $1`, [hospitalId]);
//         const totalConversations = parseInt(conversationsResult.rows[0].count);

//         // Get transcriptions count for this hospital
//         const transcriptionsResult = await executeQuery(
//             `SELECT COUNT(*) FROM transcriptions WHERE transcription_text IS NOT NULL AND hospital_id = $1`,
//             [hospitalId]
//         );
