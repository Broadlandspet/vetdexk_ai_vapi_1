...
// // In BookDemoController.js - Add this method if not exists
...
class BookDemoController {
    ...
    static async updateBookingStatus(req, res) {
        try {
            ...
            // ✅ If status is 'completed', trigger feedback email
            if (status === 'completed') {
                try {
                    await FeedbackService.markMeetingAsCompleted(id);
                    logger.info(`✅ Feedback email triggered for booking #${id}`);
                } catch (feedbackError) {
                    logger.error('Failed to send feedback email:', feedbackError);
  ...
  // Update booking status
    static async updateBookingStatus(req, res) {
        try {
            ...
            // ✅ NEW: If status is 'completed', trigger feedback email
            if (status === 'completed') {
                try {
                    await FeedbackService.markMeetingAsCompleted(id);
                    logger.info(`✅ Feedback email triggered for booking #${id}`);
