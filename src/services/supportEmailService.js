...
class SupportEmailService {
    ...
    bcc = [], 
    ticketNumber = null,
    attachments = []   // ✅ NEW: [{ filename, mimeType, buffer }]
}) {
    await this.ensureValidToken();
    ...
    if (inReplyToMessageId) {
        ...
        headerLines.push(`References: ${chain}`);
    } else if (ticketNumber) {
        const syntheticId = `<${ticketNumber}@${domain}>`;
        headerLines.push(`In-Reply-To: ${syntheticId}`);
        headerLines.push(`References: ${syntheticId}`);
    }

    if (ticketNumber) {
        headerLines.push(`X-Ticket-Number: ${ticketNumber}`);
    }

    let rawEmail;

    if (attachments.length === 0) {
        // No attachments — keep the original simple single-part shape
        ...
        for (const att of attachments) {
            const base64Content = att.buffer.toString('base64');
    ...
    logger.info(`✅ Support reply sent to ${to}, apiId: ${response.data.id}, threadId: ${response.data.threadId}, rfcMsgId: ${actualMessageId}, attachments: ${attachments.length}`);
