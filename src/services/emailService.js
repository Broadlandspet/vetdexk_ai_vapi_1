...
class EmailService {
    ...
    // Extract appointment type from transcript (subject line helper)
    ...
    // Check if appointment was actually booked (look for success message)
    ...
    // Extract booked appointment type from transcript
    ...
    // Generate dynamic subject line based on whether appointment was booked
    static generateSubjectLine(transcript, appointmentTypeFromBooking = null) {
        console.log('✅✅✅ NEW EMAIL TEMPLATE IS BEING USED! ✅✅✅');
        console.log(`   Subject generated at: ${new Date().toISOString()}`);

        const wasBooked = this.wasAppointmentBooked(transcript);
        const appointmentType = appointmentTypeFromBooking ||
            this.extractBookedAppointmentType(transcript) ||
            this.extractAppointmentType(transcript);

        if (wasBooked && appointmentType) {
            const formattedType = appointmentType.charAt(0).toUpperCase() + appointmentType.slice(1);
            console.log(`   Appointment Booked: YES - Type: ${formattedType}`);
            return `Call Logs for Appointment Summary - ${formattedType} Booked Successfully | Broadlands Pet Hospital`;
        }
    ...
    static buildEmailTemplate(data) {
        ...
        const isAppointmentBooking = data.subject && data.subject.includes('Appointment Summary');
...
<div class="detail-item">
    <span class="detail-label">Number used to book appointment:</span>
...
` : ''}
    ...
    // ============================================
    // ✅ UPDATED: Main function - sendCallSummaryEmail WITH hospital_id
    // ============================================
    static async sendCallSummaryEmail(callData) {
        ...
        try {
            ...
            const subject = this.generateSubjectLine(transcription, appointmentType);
            ...
            await this.saveEmailLog({
                ...
                hospitalId: hospitalId || null   // ✅ Passed to saveEmailLog
            });

            console.log(`   ✅ Email sent successfully with NEW TEMPLATE!`);
            return { success: true, messageId: result.messageId };
            ...
            await this.saveFailedEmailLog({
                ...
                patientId: patientId || null,
                hospitalId: hospitalId || null   // ✅ Passed to saveFailedEmailLog
            }, error.message);
    ...
    static async sendAppointmentConfirmationEmail({ toEmail, patientName, petName, appointmentType, appointmentDate, appointmentTime, callSid, appointmentId, hospitalId }) {
        try {
            const subject = `Appointment Confirmation – Broadlands Pet Hospital`;
            ...
            await this.saveEmailLog({
                ...
                callId: appointmentId ? String(appointmentId) : null,
                toEmail: toEmail,
                fromEmail: fromEmail,
                subject: subject,
                bodyHtml: html,
                ...
                callDuration: 0,
                patientId: null,
                hospitalId: hospitalId || null   // ✅ Added
            });
            ...
            await this.saveFailedEmailLog({
                ...
                toEmail: toEmail,
                fromEmail: await this.getSenderEmail(),
                subject: `Appointment Confirmation – Broadlands Pet Hospital`,
                callerName: patientName,
                callerNumber: null,
                reasonForCall: 'Appointment Confirmation',
                patientId: null,
