...
/**
 * Get OAuth2 Client – hospitalId is REQUIRED.
 * @param {number} hospitalId - hospital ID (must be provided).
 * @throws {Error} if hospitalId is not provided or credentials not found.
 */
exports.getOAuth2Client = async (hospitalId) => {
    if (!hospitalId) {
    ...
    try {
        const creds = await credentialService.getCredentials(hospitalId);
        clientId = creds.GOOGLE_CLIENT_ID;
        clientSecret = creds.GOOGLE_CLIENT_SECRET;
        refreshToken = creds.GOOGLE_GMAIL_REFRESH_TOKEN;

        if (!clientId || !clientSecret || !refreshToken) {
            throw new Error(`Incomplete Gmail credentials for hospital ${hospitalId}`);
        }
        logger.info(`Using Gmail credentials for hospital ${hospitalId}`);
    } catch (err) {
        throw new Error(`No Gmail credentials configured for hospital ${hospitalId}: ${err.message}`);
    }
...
/**
 * Send email using Gmail API – hospitalId is REQUIRED.
 */
exports.sendEmailViaGmailAPI = async ({ to, subject, html, hospitalId }) => {
    if (!hospitalId) {
        throw new Error('hospital_id is required');
    }

    try {
        const oAuth2Client = await exports.getOAuth2Client(hospitalId);
        ...
        const fromEmail = await exports.getSenderEmail(hospitalId);
        ...
        logger.info(`Email sent via Gmail API: ${result.data.id} (hospital: ${hospitalId})`);
...
/**
 * Simple email sender – hospitalId is REQUIRED.
 */
exports.sendSimpleEmail = async ({ to, subject, html, text, hospitalId }) => {
    if (!hospitalId) {
        throw new Error('hospital_id is required');
    }
    ...
    try {
        ...
        if (oAuth2Client) {
            try {
                const result = await exports.sendEmailViaGmailAPI({ to, subject, html, hospitalId });
...
/**
 * Get recipient email (admin) – hospitalId is REQUIRED.
 * @throws {Error} if hospitalId is not provided or ADMIN_EMAIL not found.
 */
exports.getRecipientEmail = async (hospitalId) => {
    if (!hospitalId) {
        throw new Error('hospital_id is required');
    }

    // First try database email_config table (may be hospital‑specific)
    ...
    try {
        const creds = await credentialService.getCredentials(hospitalId);
        if (creds.ADMIN_EMAIL) {
            logger.info(`Using ADMIN_EMAIL from hospital ${hospitalId}: ${creds.ADMIN_EMAIL}`);
            return creds.ADMIN_EMAIL;
        }
        throw new Error(`No ADMIN_EMAIL configured for hospital ${hospitalId}`);
    } catch (err) {
        throw new Error(`No recipient email configured for hospital ${hospitalId}: ${err.message}`);
    }
};

/**
 * Get sender email – hospitalId is REQUIRED.
 * @throws {Error} if hospitalId is not provided or GOOGLE_EMAIL not found.
 */
exports.getSenderEmail = async (hospitalId) => {
    if (!hospitalId) {
        throw new Error('hospital_id is required');
    }

    // First try database email_config table
    const config = await exports.getActiveConfig();
    ...
    try {
        const creds = await credentialService.getCredentials(hospitalId);
        if (creds.GOOGLE_EMAIL) {
            logger.info(`Using GOOGLE_EMAIL from hospital ${hospitalId}: ${creds.GOOGLE_EMAIL}`);
            return creds.GOOGLE_EMAIL;
        }
        throw new Error(`No GOOGLE_EMAIL configured for hospital ${hospitalId}`);
    } catch (err) {
        throw new Error(`No sender email configured for hospital ${hospitalId}: ${err.message}`);
    }
};
...
exports.sendCallSummaryEmail = async (callData) => {
    ...
    if (!hospitalId) {
        throw new Error('hospital_id is required to send email');
    }
    ...
    try {
        ...
        const toEmail = await exports.getRecipientEmail(hospitalId);
        const fromEmail = await exports.getSenderEmail(hospitalId);
        ...
        await exports.saveFailedEmailLog({
            callSid,
            callId,
            toEmail: await exports.getRecipientEmail(hospitalId).catch(() => null),
            fromEmail: await exports.getSenderEmail(hospitalId).catch(() => null),
...
exports.sendAppointmentConfirmationEmail = async ({ toEmail, patientName, petName, appointmentType, appointmentDate, appointmentTime, callSid, appointmentId, hospitalId }) => {
    ...
    try {
        ...
        console.log(`✅ Appointment confirmation email sent to patient: ${toEmail} (hospital_id: ${hospitalId})`);
        return { success: true, messageId: result.messageId };
