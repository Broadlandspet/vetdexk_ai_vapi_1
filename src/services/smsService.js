...
const axios = require('axios');
const logger = require('../utils/logger');
const credentialService = require('./credentialService'); // ← New import
 ...
 * Send an SMS via Vapi's chat endpoint
 * @param {Object} params
 * @param {number|string} params.hospitalId - Hospital ID (fetches Vapi credentials from vet_desk_ai_crendatials table)
 * @param {string} params.to - Recipient phone number (will be auto-formatted to E.164)
 * @param {string} params.message - Text message to send
 * @param {string} [params.transportType='twilio.sms'] - Transport type
 * @param {boolean} [params.useLLMGenerated=false] - Whether to use LLM generation
...
exports.sendSms = async function({ hospitalId, to, message, transportType = 'twilio.sms', useLLMGenerated = false }) {
    try {
        ...
        // ─── Fetch Vapi credentials from encrypted table via credentialService ──
        let creds;
        try {
            creds = await credentialService.getCredentials(hospitalId);
        } catch (err) {
            // credentialService throws "No credentials found for hospital_id: ..." if not found
            // Re-throw with a consistent error message to match the old behavior
            if (err.message && err.message.includes('No credentials found')) {
                throw new Error(`Hospital ${hospitalId} not found or credentials not configured`);
            }
            throw err; // re-throw other errors
        }

        // Extract the needed fields – keys are uppercase thanks to credentialService
        const vapiApiKey = creds.VAPI_API_KEY;
        const assistantId = creds.VAPI_ASSISTANT_ID;
        const phoneNumberId = creds.VAPI_PHONE_NUMBER_ID;

        // Validate they exist (credentialService already decrypts, but may return null if missing)
        if (!vapiApiKey) {
            throw new Error('Vapi API key not configured for this hospital');
        }
        if (!assistantId) {
