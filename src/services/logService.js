...
/**
 * Find or create a conversation with optional hospital_id
 */
exports.findOrCreateConversation = async (fromNumber, toNumber, hospitalId = null) => {
    try {
        let findResult;
        if (hospitalId) {
            findResult = await executeQuery(
...
/**
 * Create a new call record with hospital_id
 */
exports.createCall = async (callData) => {
    try {
        const conversationId = await exports.findOrCreateConversation(
            callData.caller_number,
            callData.callee_number,
            callData.hospital_id || null
        );
...
/**
 * Update call fields (status, menu_digit)
 */
exports.updateCall = async (callSid, updates) => {
    try {
        const allowedFields = ['call_status', 'menu_digit'];
        const setClauses = [];
        const values = [];
...
/**
 * Update call with transcription text and recording URL
 */
exports.updateCallWithTranscription = async (callSid, transcriptionText, recordingUrl = null) => {
    try {
        const call = await exports.getCallBySid(callSid);
        if (!call) {
            logger.error(`Call not found for transcription: ${callSid}`);
            return;
        }
        ...
        await exports.updateCall(callSid, { call_status: 'completed' });
...
/**
 * Append a transcript line to an existing transcript
 */
exports.updateCallWithTranscript = async (callId, transcriptLine) => {
    try {
        const existingResult = await executeQuery(
            `SELECT id, transcription_text FROM transcriptions WHERE call_id = $1`,
            [callId]
...
/**
 * Add a log entry for a call
 */
exports.addLog = async (callId, eventType, eventData) => {
    try {
        await executeQuery(
            `INSERT INTO ezy_vet_call_logs (call_id, event_type, event_data)
             VALUES ($1, $2, $3)`,
...
/**
 * Get a call by its Twilio CallSID
 */
exports.getCallBySid = async (callSid) => {
    try {
        const result = await executeQuery(
            `SELECT c.*, 
                    jsonb_agg(DISTINCT t.*) FILTER (WHERE t.id IS NOT NULL) as transcriptions,
...
/**
 * Get all logs for a given call
 */
exports.getCallLogs = async (callId) => {
    try {
        const result = await executeQuery(
            `SELECT * FROM ezy_vet_call_logs 
             WHERE call_id = $1 
 ...
 */
exports.getAllCalls = async (filters = {}) => {
    try {
        let query = `
            SELECT c.*, 
                   jsonb_agg(DISTINCT t.*) FILTER (WHERE t.id IS NOT NULL) as transcriptions,
...
/**
 * Save recording information for a call
 */
exports.saveRecordingInfo = async (callSid, recordingUrl, recordingSid, recordingDuration = null) => {
    try {
        const call = await exports.getCallBySid(callSid);
        if (!call) {
            logger.error(`Call not found for recording: ${callSid}`);
 ...
 */
exports.getConversationByNumbers = async (fromNumber, toNumber) => {
    try {
        const result = await executeQuery(
            `SELECT c.*,
                    jsonb_agg(DISTINCT ca.*) FILTER (WHERE ca.id IS NOT NULL) as calls
